...
abstract class PointGraph extends GridGraph
{
   ...
   */
    private function CreateMarker(
        ...
        $id = $this->NewID();
        $marker = ['id' => $id, 'cursor' => 'crosshair', 'fill' => $fill];
   ...
   */
    protected function FormatTooltip(&$item, $dataset, $key, $value)
    {
        ...
        static::numString($key) . $this->units_tooltip_key : $key;
        $text .= ', ' . $this->units_before_tooltip . static::numString($value) .
        $this->units_tooltip;
        return $text;
    }
}
